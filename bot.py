...
def makeDecisions(gems):
    ...
    # Step 4: Sort the decision list from top right to bottom left (alternating) to minimize conflicts
    finalResult = []
    flag = True
    
    while len(result) > 0:
        decision = result.pop() if flag else result.pop(0)
        flag = not flag
        finalResult.append(decision) 
    
    
    # Step 5: Return the decision list
    return finalResult
